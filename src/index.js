...
const getMovieData = async uri => {
	...
	return {
		...
		title: $('#mv-info .mvic-desc h3').text().trim(),
		genres: $('#mv-info .mvic-info .mvici-left p').eq(0).find('a').map((i, el) => $(el).text().trim()).get(),
		actors: $('#mv-info .mvic-info .mvici-left p').eq(1).find('a').map((i, el) => $(el).text().trim()).get(),
		director: $('#mv-info .mvic-info .mvici-left p').eq(2).find('a').map((i, el) => $(el).text().trim()).get(),
		country: $('#mv-info .mvic-info .mvici-left p').eq(3).find('a').map((i, el) => $(el).text().trim()).get(),
		duration: $('#mv-info .mvic-info .mvici-right p').eq(0).text().replace(/[^\d]/gi, ''),
		quality: $('#mv-info .mvic-info .mvici-right p span.quality').text().trim(),
		year: $('#mv-info .mvic-info .mvici-right p').eq(2).text().replace(/[^\d]/gi, ''),
		imdb: $('#mv-info .mvic-info .mvici-right p').eq(3).text().replace(/[^\d|^\.]/gi, ''),
		keywords: $('#mv-keywords a').map((i, el) => $(el).text().trim()).get(),
		trailer: _.get(/"(https:\/\/www.youtube.com\/embed\/.+?)"/gi.exec($('.modal-trailer').next('script').html()), '[1]', ''),
...
const getEpisodes = async (url) => {
	...
	return $('.btn-eps.ep-item').map((i, el) => $(el).data('id')).get();
}
...
const getSources = async (url) => {
	...
	const sources = await Promise.all(tokens.map(async ({ eid, x, y }) => {
...
(async () => {
	const urls = await getMoviesUrls();
	urls.map(async (url) => {
		const sources = await getSources(url);
		console.log(_.get(sources, '[1].playlist', null))
	})
